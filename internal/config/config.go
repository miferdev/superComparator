...
func Load() Config {
	...
	c := Config{
		...
		Candidates:  envInt("SUPERCOMPARATOR_CANDIDATES", 6),
		RankPool:    envInt("SUPERCOMPARATOR_RANK_POOL", 15),
		MinScore:    envFloat("SUPERCOMPARATOR_MIN_SCORE", 0),
		MaxPriceAge: time.Duration(envInt("SUPERCOMPARATOR_MAX_PRICE_AGE_DAYS", 0)) * 24 * time.Hour,
